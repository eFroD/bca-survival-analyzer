...
class BCASurvivalAnalyzer:
    ...
    def univariate_cox_regression(self, columns, verbose=False):
        return perform_univariate_cox_regression(self.df, columns, self.standardize, verbose=verbose)

    def kaplan_meier_plot(self, column, split_strategy='median', fixed_value=None, output_path=None):
        return generate_kaplan_meier_plot(self.df, column, split_strategy, fixed_value, output_path=output_path)
